/**
 * Remap CyberpunkElf body → HighPriestess grid via TRANSLATION ONLY.
 * Body size is preserved (no scaling) so the same body can be shared
 * across models for dress-up. Only center position is aligned.
 *
...
// Body-only grids (from voxelize_body_only.py)
const ceBodyGrid = JSON.parse(fs.readFileSync(path.join(BASE, 'public/box-compare/ce_body_grid.json'), 'utf8'));
const hpBodyGrid = JSON.parse(fs.readFileSync(path.join(BASE, 'public/box-compare/hp_body_grid.json'), 'utf8'));

// Full model grids (from blender_voxelize.py with body-only bbox deformation)
...
// Body deformed centers (for translation)
...
// Translation offset: CE body center → HP body center (no scaling)
const translation = [
...
// Fine-tune: slightly more forward (positive Y in deformed space)
...
// Read CE body from the FULL model voxelization (has skin fill under clothing)
...
// Read HP body (original from Blender, before remap) for skin color palette
// Use the full model .vox which has HP's native palette
const hpFull = readVox(path.join(BASE, 'public/box3-new/highpriestess_blender_rigged.vox'));
const hpPalette = hpFull.palette;

// Read HP body from old box3 to identify skin color indices
const hpOldBody = readVox(path.join(BASE, 'public/box3/highpriestess_blender_rigged_body.vox'));
// Find most common HP body colors (skin tones)
const hpBodyColors = {};
...
// Build CE palette index → HP palette index mapping
// For each CE color, find nearest HP skin color by luminance
function colorDist(a, b) {
  return Math.abs(a.r - b.r) + Math.abs(a.g - b.g) + Math.abs(a.b - b.b);
}

// Build luminance-sorted HP skin colors for mapping
...
// Map CE color indices to HP equivalents (by relative luminance rank)
const ceUsedColors = {};
...
// Create mapping: CE luminance rank → HP luminance rank
const colorMap = {};
...
const ceVs = ceFullGrid.voxel_size;
const hpVs = hpFullGrid.voxel_size;
...
for (const v of ceBody.voxels) {
  ...
  // Translate to HP space (no scaling — body size preserved)
  const hx = wx + translation[0];
  const hy = wy + translation[1];
  const hz = wz + translation[2];

  // HP deformed world → HP voxel
