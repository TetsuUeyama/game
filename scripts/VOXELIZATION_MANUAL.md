...
**ボクセル化する3Dモデルは必ずTポーズ（両腕を水平に伸ばした状態）であること。**

Mixamoモーションデータは「レストポーズ（Tポーズ）からの回転差分（delta quaternion）」として記録されている。ボクセルモデルの初期ポーズがTポーズと一致していないと、モーション再生時にオフセットが生じ、腕が上がりすぎる・内側にめり込む等の問題が発生する。

Blenderでの確認方法:
1. FBX/Blendファイルを開く
2. Armature（ボーン）が表示されている場合、両腕が真横に伸びていることを確認
3. Aポーズ（腕が斜め下）の場合、ポーズモードでTポーズに修正してからエクスポート
...
7. **Tポーズ必須**: ボクセル化元の3Dモデルは必ず**Tポーズ**（両腕を真横に伸ばした状態）でエクスポートすること。Mixamoモーションデータはレストポーズ（Tポーズ）からの差分として記録されているため、Tポーズ以外でボクセル化するとモーション再生時に腕の角度や関節の向きがずれる
...
- [ ] 元モデルがTポーズでエクスポートされているか（モーション再生の前提条件）
...
> 詳細なロードマップ: `scripts/PIPELINE_ROADMAP.md` を参照

### 背景

現在のパイプラインはAI依存度が高く（約90%）、パラメータ調整・スクリプト作成・トラブル対応をAIが担当している。
以下の5フェーズで、ユーザーが自力でキャラクター制作を完結できるシステムに進化させる。

### フェーズ概要

| Phase | 内容 | AI依存度 |
|---|---|---|
| 1 | FBXインポート & パーツ分類UI | 90% → 70% |
| 2 | テンプレートシステム（髪キャップ・衣装ベース） | → 30% |
| 3 | 左右対称性強制 & Tポーズ保証 | 品質向上 |
| 4 | 体型適応フィッティング | → 10% |
| 5 | リアルタイムパラメータ調整UI | → 5% |
...
#### テンプレートシステム
- **ヘアキャップ**: チビ頭にフィットした球殻状の下地。髪をキャップに「植毛」する方式で、従来の頭頂拡大・前髪生成等の後処理が不要になる
- **衣装シェル**: Body表面から+1〜2ボクセル外側に自動生成したシェル。衣装はシェル上に投影されるため、膨張・干渉修正が不要になる
- **テンプレート + 自由編集**: Body表面に接する部分はテンプレートが形状を保証し、離れた部分は3Dモデルの形状をそのまま使用。非対称パーツや例外的な形状にも対応

#### 体型適応フィッティング
- 衣装を「Body表面からの相対距離」で定義
- 異なる体型のBodyに対して、同じ相対距離で再配置 → 自動フィット
- キャラAの衣装をキャラBに着せ替え可能

#### 対称性 & Tポーズ
- ボクセル化後に左半分をX軸ミラーコピーして完全対称化
- Blenderエクスポート時にTポーズを自動検出、Aポーズの場合は自動修正

### 現行パイプラインとの関係

次世代パイプラインが完成するまでは、現行のパイプライン（本マニュアルの Step 0〜4）をそのまま使用する。
次世代パイプラインは現行パイプラインを**置き換える**ものであり、両方を併用する必要はない。
ただし、出力形式（.vox + bone-config.json + parts-manifest.json + motion.json）は変わらないため、
ゲーム側への影響はない。
