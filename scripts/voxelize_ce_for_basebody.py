"""
Voxelize CyberpunkElf clothing/accessories/hair for use with BasicBodyFemale base body.

Usage:
  blender --background --python voxelize_ce_for_basebody.py -- <input.blend> <output_dir> [voxel_size] [hair_thr]

This script:
1. Opens the CyberpunkElf model
2. Identifies all mesh parts (body, clothing, accessories, hair)
3. Voxelizes each non-body part using the same grid as BasicBodyFemale
4. Hair is voxelized with a custom threshold (default 0.3)
5. Outputs per-part .vox files + parts.json + grid.json
"""
import bpy
import bmesh
import sys
...
VOXEL_SIZE = float(args[2]) if len(args) > 2 else 0.007  # Match BasicBodyFemale
HAIR_THR = float(args[3]) if len(args) > 3 else 0.3
...
# ========================================================================
# Load BasicBodyFemale grid to use same coordinate system
# ========================================================================
...
if os.path.exists(BASEBODY_GRID_PATH):
    ...
    BASE_BB_MIN = Vector(base_grid["bb_min"])
    BASE_BB_MAX = Vector(base_grid["bb_max"])
    BASE_VOXEL_SIZE = base_grid["voxel_size"]
    USE_BASE_GRID = True
else:
...
BODY_KEYWORDS = ['body']
HAIR_KEYWORDS = ['hair']
EXCLUDE_KEYWORDS = ['collision', 'modular', 'penis', 'pubes', 'cs_']
...
for obj in bpy.context.scene.objects:
    ...
    if any(kw in name_lower for kw in BODY_KEYWORDS) and 'bra' not in name_lower:
        ...
        # Classify as clothing or accessory based on name
...
# ========================================================================
# Ensure visibility and disable MASK modifiers
# ========================================================================
...
# ========================================================================
# Grid setup - use BasicBodyFemale grid
# ========================================================================
if USE_BASE_GRID:
    bb_min = BASE_BB_MIN
    bb_max = BASE_BB_MAX
...
# ========================================================================
# Voxelize a set of mesh objects
# ========================================================================
def voxelize_meshes(mesh_objects, threshold_mult=1.2):
    """Voxelize a list of mesh objects and return list of (x,y,z,color_index)."""
    depsgraph = bpy.context.evaluated_depsgraph_get()

    # Build combined BVH from all mesh objects
    ...
    for obj in mesh_objects:
        ...
        bmesh.ops.triangulate(bm_part, faces=bm_part.faces)
        ...
        # Merge into combined
        # We need to manually add verts and faces
...
# ========================================================================
# Process each category (skip body - we use BasicBodyFemale segments)
# ========================================================================
os.makedirs(OUT_DIR, exist_ok=True)

parts_manifest = []
all_palettes = {}  # Accumulate palettes across parts
...
# ========================================================================
# Write grid.json (same as BasicBodyFemale)
# ========================================================================
grid_meta = {
    "voxel_size": voxel_size,
    "gx": sx, "gy": sy, "gz": sz,
...
for p in parts_manifest:
    print(f"    {p['category']}/{p['key']}: {p['voxels']} voxels")
