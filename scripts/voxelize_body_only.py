...
args = argv[idx + 1:]
NO_DEFORM = '--no-deform' in argv  # チビ変形無効化フラグ
pos_args = [a for a in args if not a.startswith('--')]
INPUT_PATH = pos_args[0]           # 入力モデルファイル
OUT_PATH = pos_args[1]             # 出力VOXパス
RESOLUTION = int(pos_args[2]) if len(pos_args) > 2 else 100  # グリッド解像度
        ...
        for kb in obj.data.shape_keys.key_blocks:
            if kb.name.startswith('Clothes_'):
                if KEEP_SHAPEKEYS:
                    ...
                    kb.value = 0.0
    # 物理モディファイアの除去（バックグラウンドモードでは動作しない）
...
thr = max(voxel_size * 1.2, base_voxel * 1.2)  # BVH距離閾値
...
for vz in range(gz):
    ...
    for vx in range(gx):
        for vy in range(gy):
            dp = Vector((grid_origin.x + (vx + 0.5) * voxel_size, grid_origin.y + (vy + 0.5) * voxel_size, grid_origin.z + (vz + 0.5) * voxel_size))
            op = inv_deform(dp)
...
for vz_trim in range(min_vz, min_vz + sole_trim_layers):
    ...
    for pos in to_remove:
        ...
        removed_sole += 1
print(f"  Sole trim: removed {removed_sole} voxels from bottom {sole_trim_layers} layers")
...
# ボディから突出した目ボクセルを削除、表面の目位置をスキン色で埋める
