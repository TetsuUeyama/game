...
export const PlayerInfo = ({ player, showRole = false, rolePosition = 'below' }: PlayerInfoProps) => {
  ...
  const hasRole = player.hand?.role && player.hand.role !== "役を判定中...";
  const attackDisplay = player.attackAction || "balance";
  const defenseDisplay = player.defenseAction || "balance";
  
  console.log('PlayerInfo Debug:', {
    playerName: player.character.name,
    hasRole,
    role: player.hand?.role,
    attackAction: player.attackAction,
    defenseAction: player.defenseAction,
    showRole,
    rolePosition,
    roleDisplay: !!roleDisplay,
    shouldShowSelectionAbove: rolePosition === 'above' && (!showRole || !roleDisplay),
    shouldShowSelectionBelow: rolePosition === 'below' && (!showRole || !roleDisplay)
  });
