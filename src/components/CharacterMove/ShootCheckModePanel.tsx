...
/**
 * シュートチェックモードパネル
 * GameScene を使用して実際のゲームロジックでシュートをテスト
 */
export function ShootCheckModePanel({ gameScene, onClose }: ShootCheckModePanelProps) {
  const shootCheckControllerRef = useRef<ShootCheckController | null>(null);
  ...
  // シュートチェックを開始
  const startShootCheck = useCallback(() => {
    if (!gameScene || !selectedPlayerId) return;

    // 初期位置をセル座標からワールド座標に変換
    const startCol = checkMode === 'single' ? selectedCol : 'H';
    const startRow = checkMode === 'single' ? selectedRow : 15;
    const startWorldPos = FieldGridUtils.cellToWorld(startCol, startRow);

    if (!startWorldPos) {
      console.error('[ShootCheckModePanel] 座標変換に失敗しました');
      return;
    }
    ...
    if (!shootingController) {
      console.error('[ShootCheckModePanel] ShootingController が初期化されていません');
      return;
    }

    // シュートチェックコントローラーを作成
    shootCheckControllerRef.current = new ShootCheckController(
      shooter,
      gameScene.getBall(),
      gameScene.getField(),
      shootingController,
    ...
    // ゴール時のコールバックを設定（シュート成功を通知）
    shootingController.setOnGoalCallback(() => {
      if (shootCheckControllerRef.current) {
        shootCheckControllerRef.current.notifyGoalScored();
      }
    });

    // コールバック設定
    shootCheckControllerRef.current.setOnProgressCallback((p) => {
      setProgress(p);
    });

    shootCheckControllerRef.current.setOnCellCompleteCallback((result) => {
      setResults((prev) => [...prev, result]);
    });

    shootCheckControllerRef.current.setOnCompleteCallback((finalResults) => {
    ...
    // モードに応じてシュートチェックを開始
    if (checkMode === 'single') {
      shootCheckControllerRef.current.startSingleCell(selectedCol, selectedRow);
    } else {
      shootCheckControllerRef.current.start();
    }

    console.log('[ShootCheckModePanel] シュートチェックを開始しました（GameScene使用）');
  }, [
    gameScene,
    selectedPlayerId,
    players,
  ...
  useEffect(() => {
    ...
    return () => {
      // クリーンアップ
      if (shootCheckControllerRef.current) {
        shootCheckControllerRef.current.dispose();
        shootCheckControllerRef.current = null;
      }
    };
  }, [phase, startShootCheck]);

  // 更新ループ（シュートチェックコントローラーとゲームシーンの更新）
  useEffect(() => {
    if (phase !== 'running' || !shootCheckControllerRef.current || !gameScene) return;
    ...
    const updateLoop = (currentTime: number) => {
      ...
      // ShootingController の更新（シュートアクション、ゴール判定など）
      if (shootingController) {
        shootingController.update(deltaTime);
      }
      ...
      // シュートチェックコントローラーの更新
      if (shootCheckControllerRef.current) {
        shootCheckControllerRef.current.update(deltaTime);
      }
  ...
  const handleStart = useCallback(() => {
    ...
    setResults([]);
    setPhase('running');
  }, [selectedPlayerId, gameScene]);

  // 中断
  const handleAbort = useCallback(() => {
    if (shootCheckControllerRef.current) {
      shootCheckControllerRef.current.abort();
    }
    setPhase('completed');
  }, []);
        ...
        <>
          ...
          {(phase === 'completed' || results.length > 0) && (
            <div className={`pointer-events-auto ${
              phase === 'running'
                ? 'absolute bottom-20 left-4 w-64'
                : 'flex-1 p-4'
            }`}>
              <ShootCheckHeatmap
                results={results}
          ...
          {/* ホバー情報 */}
          {hoveredResult && (
            <div className="absolute top-20 left-4 bg-black/80 p-4 rounded-lg text-white pointer-events-auto">
          ...
          {showTable && phase === 'completed' && (
            <div className="absolute top-20 right-0 bottom-20 w-96 bg-gray-800/90 backdrop-blur-sm border-l border-gray-700 overflow-y-auto pointer-events-auto">
