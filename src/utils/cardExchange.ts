...
export const handleCardExchange = (
  ...
  setEnemyCurrentHp: (hp: number | ((prev: number) => number)) => void
) => {
  ...
  // 捨てたカードを手札から削除
  const newPlayerCards = state.playerCards.filter((_, index) => !state.discardedCardIndices.has(index))
  ...
  for (let i = 0; i < discardedCount; i++) {
    ...
    const newCard: Card = {
      ...
      rank: randomRank,
      image: `/images/cards/${randomSuit}_${randomRank}.png`
    }
...
export const handleRemoveCard = (
  ...
  if (slotCards && slotCards.length > 0) {
    ...
    newSlotAttacks[slotIndex] = {
      ...newSlotAttacks[slotIndex],
      damageBonus: Math.max(0, newSlotAttacks[slotIndex].damageBonus - 1)
    }
    actions.setSlotAttacks(newSlotAttacks)
  }
}
