...
const BASE = '/api/box5/cyberpunkelf_arp';
...
/** CyberpunkElf のパーツ構成。Body (顔/体/目 統合)、Hair 別 mesh、Default Outfit 14 mesh */
const PART_GROUPS: Array<{ group: string; parts: Array<{ key: string; label: string; color: string }> }> = [
  {
    group: 'Body & Face',
    parts: [
      { key: 'body',  label: 'Body (顔/体/目 統合)', color: '#faa' },
      { key: 'hair',  label: 'Hair (別 mesh)',       color: '#fa8' },
    ],
  },
  {
    group: 'Default Outfit (Suit_01)',
    parts: [
      { key: 'bra',      label: 'Bra',     color: '#f88' },
      { key: 'hat',      label: 'Hat',     color: '#fa8' },
      { key: 'jacket',   label: 'Jacket',  color: '#f88' },
      { key: 'necktie',  label: 'Necktie', color: '#f8f' },
      { key: 'panties',  label: 'Panties', color: '#fc8' },
    ],
  },
  {
    group: 'Default Outfit (Suit_02)',
    parts: [
      { key: 'armband',            label: 'Armband',            color: '#8ff' },
      { key: 'garter_straps',      label: 'Garter Straps',      color: '#aaf' },
      { key: 'gloves',             label: 'Gloves',             color: '#8af' },
      { key: 'hip_plate',          label: 'Hip Plate',          color: '#aff' },
      { key: 'hologram_a',         label: 'Hologram A',         color: '#aaf' },
      { key: 'hologram_b',         label: 'Hologram B',         color: '#aaf' },
      { key: 'hologram_projector', label: 'Hologram Projector', color: '#cca' },
      { key: 'leggings',           label: 'Leggings',           color: '#8ff' },
      { key: 'leggings_001',       label: 'Leggings.001',       color: '#8ff' },
    ],
  },
];
...
// suit 系は ARMOR_VARIATIONS で動的生成 (各 mesh 共通の blue/white)
const ARMOR_VARIATIONS = ['blue', 'white'] as const;
const ARMOR_PART_KEYS = [
  // Suit_01
  'bra', 'hat', 'jacket', 'necktie', 'panties',
  // Suit_02
  'armband', 'garter_straps', 'gloves', 'hip_plate',
  'hologram_a', 'hologram_b', 'hologram_projector',
  'leggings', 'leggings_001',
];
const EFFECT_SLOTS_PER_PART: Record<string, string[]> = {
  body: [
    'blush_color',
    'skin_body_dark', 'skin_head_dark',
    'eyes_red', 'eyes_turquoise',
  ],
  hair: ['hair_dark', 'hair_white'],
  // 各 armor mesh は <variant>_color slot を持つ (blue/white の 2 種)
  ...Object.fromEntries(ARMOR_PART_KEYS.map(k => [k, ARMOR_VARIATIONS.map(v => `armor_${v}`)])),
};

// Skin Color バリアント名 (default は素のベース = light skin)
const SKIN_COLOR_OPTIONS = [
  { id: 'default',  label: 'Default (Light)' },
  { id: 'dark',     label: 'Dark Skin' },
] as const;
...
// Eyes Color (デフォルトは BaseColor ベイク済み)
...
export default function CyberpunkElfPreviewPage() {
  ...
  // Tattoo は CyberpunkElf にないので削除 (state は残し warning 抑止のため _ で参照)
  const [tattooSlider] = useState(0); void tattooSlider;
  ...
  useEffect(() => {
    ...
    // CyberpunkElf は dark variant のみ (default = light skin)
    ...
    // Eyes Color (body 内の Eyes material region のみ、default = base color)
    ...
    for (let i = 0; i < numVerts; i++) {
      ...
      // 2. Eyes Color swap (body 内の Eyes material のみ)
      if (eyesVar) {
  ...
  }, [skinColor, eyesColor, blushSlider, showEyesReal, showEyesOverlay, showInternalMouth, partInfo]);
    ...
    <div style={{ display: 'flex', height: '100vh',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '10px 14px', background: 'rgba(0,0,0,0.3)',
          ...
          <span style={{ color: '#8af', fontSize: 13, fontWeight: 'bold' }}>
            CyberpunkElf (ARP MustardUI) Preview
          </span>
          <span style={{ fontSize: 11, color: '#888', marginLeft: 12 }}>
            game-assets/vox-model/box5/cyberpunkelf_arp/
          </span>
          {grid && (
      ...
      <div style={{ width: 280, padding: 12, background: 'rgba(0,0,0,0.4)',
        ...
        <div style={{ fontSize: 10, color: '#666', marginTop: 4 }}>
          ※ CyberpunkElf には Tattoo / Wet エフェクトなし
        </div>
        ...
        <div style={{ marginTop: 20, padding: 8, fontSize: 10,
          ...
          Files under <b>game-assets/vox-model/box5/cyberpunkelf_arp/</b>.<br/>
