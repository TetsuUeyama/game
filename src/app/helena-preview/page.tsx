...
/** Helena (DAZ G8F + DOA outfits) のパーツ構成。
 * - Body & Face: Helena_Base (DAZ G8F 14 material 統合) + Hair Mesh + Eyelashes
 * - Outfit: 4 種類 (Bodysuit / Dark Prison / Rise and Shine / Qipao) - 各 outfit mesh 群
 * MustardUI スライダーなし (シンプル UI、ON/OFF のみ)
 */
const PART_GROUPS: Array<{ group: string; parts: Array<{ key: string; label: string; color: string }> }> = [
  {
    group: 'Body & Face',
    parts: [
      { key: 'body',       label: 'Body (Helena_Base)', color: '#faa' },
      { key: 'hair',       label: 'Hair Mesh',          color: '#fa8' },
      { key: 'eyelashes',  label: 'Eyelashes',          color: '#aaa' },
    ],
  },
  {
    group: 'Outfit: GCC DOA Bodysuit',
    parts: [
      { key: 'bodysuit', label: 'Bodysuit', color: '#88f' },
    ],
  },
  {
    group: 'Outfit: GCC DOA Dark Prison',
    parts: [
  ...
  {
    group: 'Outfit: GCC DOA Rise and Shine',
    parts: [
      { key: 'rs_panties',      label: 'Panties',      color: '#fc8' },
      { key: 'rs_shirt_normal', label: 'Shirt Normal', color: '#f8f' },
      { key: 'rs_shirt_nude',   label: 'Shirt Nude',   color: '#f8f' },
    ],
  },
  {
    group: 'Outfit: Qipao',
...
// Helena は MustardUI スライダーなし → effect slot は空
// 各 outfit mesh は variant texture なし → ARMOR_VARIATIONS は空
const ARMOR_VARIATIONS = [] as const;
const ARMOR_PART_KEYS = [
  'bodysuit', 'dark_prison_a', 'dark_prison_b',
  'rs_panties', 'rs_shirt_normal', 'rs_shirt_nude',
  'qipao', 'qipao_panty', 'qipao_shoe', 'qipao_sock',
];
const EFFECT_SLOTS_PER_PART: Record<string, string[]> = {};  // 全 part effect なし (シンプル UI)

// Helena は Color Variation スライダーなし → SkinColor/HairColor/EyesColor/ArmorColor は default のみ
...
export default function HelenaPreviewPage() {
  ...
  const [visible, setVisible] = useState<Record<string, boolean>>(
    Object.fromEntries(PARTS.map(p => [p.key,
      // 初期表示: body / hair / eyelashes のみ ON、各 outfit はユーザーが選択
      p.key === 'body' || p.key === 'hair' || p.key === 'eyelashes'
    ]))
  );
