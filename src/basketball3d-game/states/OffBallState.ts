 ...
 */
export class OffBallState extends PlayerState {
  readonly type = PlayerStateType.OFF_BALL;
  ...
  onEnter(player: Player, context: GameContext): void {
    this.log(player, "Entering OFF_BALL state - teammate has the ball");
  }

  update(player: Player, context: GameContext): void {
    // TODO: Phase 2でチームプレイ時のポジショニングを実装
    // 現在は何もしない
  }
