...
/**
 * SG / SPACER: corner positioning with maximum spacing emphasis
 */
export function moveSpacer(
  tgt: SimMover,
  dest: { x: number; z: number } | null,
  reevalTimer: number,
  dt: number,
  ...
  const role = ROLE_ASSIGNMENTS.targets[4];
  reevalTimer -= dt;

  // Pressure check: slide to zone edge if obstacle approaches
  let underPressure = false;
  for (const ob of obstacles) {
    if (dist2d(tgt.x, tgt.z, ob.x, ob.z) < OPEN_THRESHOLD) {
      underPressure = true;
  ...
  if (reevalTimer <= 0 || !dest || atDest || underPressure) {
    if (underPressure) {
      // Slide to zone edge away from pressure
      dest = {
        x: role.zone.xMax - 0.2,
        z: role.zone.zMax - 0.2,
      };
    } else {
      dest = findOpenSpaceInZone(role.zone, launcher, obstacles, otherTargets, role.homeX, role.homeZ, 25);
    }
    reevalTimer = role.reevalInterval + Math.random() * 0.5;
  }
